...
function shortenTitle(title: string, maxLength = 22): string {
  return title.length > maxLength ? `${title.slice(0, maxLength - 1)}…` : title;
}

function isTitleTruncated(title: string, maxLength = 22): boolean {
      ...
      <div className="grid gap-2 overflow-visible">
        {liveBooks.map((book) => (
          <Link
            ...
            href={book.href}
            className="group relative flex h-[118px] w-full -translate-x-[1px] gap-4 border-2 border-border bg-bg p-4 transition-all duration-150 hover:z-10 hover:border-accent hover:bg-accent-dim focus-visible:z-10 focus-visible:border-accent focus-visible:bg-accent-dim rounded-md"
          >
